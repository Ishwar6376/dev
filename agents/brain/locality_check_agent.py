...
def locality_check_agent(state: AgentState):
    ...
    """
    category_endpoints = {
        "WATER": "/locality/waterCheck",
        "INFRASTRUCTURE": "/locality/infraCheck",
        "WASTE": "/locality/wasteCheck",
        "ELECTRICITY": "/locality/electricityCheck"
    }
        ...
        if data.get("duplicateFound") is True:
            ...
            if verify_image_similarity(current_user_url, existing_backend_url):
                return {
                    ...
                    "locality_email": report_data.get("email"),
                }
            else:
                print("Visual check failed. Saving as new.")
                return {"tool": "SAVE", "locality_imageUrl": None, "locality_userId": None}

        return {"tool": "SAVE", "locality_imageUrl": None, "locality_userId": None}
    except Exception as e:
        print(f"Locality check failed: {e}")
        return {"tool": "SAVE"}
def save_report_tool(state: AgentState):
    """
    Creates a NEW report in the database using the route stored in state.
    Expected routes in state['route']: 
    - "reports/waterReports"
    - "reports/wasteReports"
    - "reports/infrastructureReports"
    - "reports/electricityReports"
